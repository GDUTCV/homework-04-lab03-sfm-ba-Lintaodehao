...
def compute_ba_residuals(parameters: np.ndarray, intrinsics: np.ndarray, num_cameras: int, points2d: np.ndarray,
    ...
    h_2d = np.concatenate((points2d,np.ones((points2d.shape[0],1))),axis=1)
    h_3d = np.concatenate((points3d[points3d_idxs],np.ones((points3d[points3d_idxs].shape[0], 1))),axis=1)

    p_2d = np.einsum('kj,klj->kl',h_3d, proj_mtx)
    p_2d = p_2d/np.repeat(p_2d[:,2].reshape(p_2d.shape[0],1),3,axis=1) # Normalizing

    residuals = np.linalg.norm(h_2d - p_2d,axis=1)
